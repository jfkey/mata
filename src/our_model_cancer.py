...
class OurNNTrainer(object):
    ...
    def process_dataset(self):
        ...
        if self.args.nonstruc:
            self.num_labels = self.all_graphs.num_features
        else:
            ...
            self.all_graphs.transform = transform
            self.num_labels = self.all_graphs.num_features + self.args.max_degree + self.args.random_walk_step
    ...
    def create_batches(self):
        ...
        for i in range(self.args.batch_size):
            item = data_list[i]
            g1 = self.all_graphs[item['rowid']]
            g2 = self.all_graphs[item['colid']]
            ged = item['ged']
    ...
    def score(self, test=True):
        ...
        for i in range(rownum):
            ...
            for start_idx in range(0, len(row_graphs), self.args.batch_size):  # 从0到len(self.training_graphs)间隔 batch size
                ...
                pred_ged, _, _, simmat3 = self.model(new_data)
    ...
    def do_astar(self, sim_mat, new_data):
        ...
        for b in range(batch_num):
            ...
            beam_size = get_beam_size(n1, n2, e1, e2, self.args.dataset)
            topk = self.args.topk
            if topk == 0: topk = 1
